...
| r1_v14_defensive | 259621 | 10,443.78 | 7,292 | 3,152 | v12 + asymmetric quoting ("lean out"): when crash_mode + \|pos\|≥60, set accumulation-side cap to 0. **Byte-identical to v12/v13 on website** (never fires on real data). Synthetic −4,736 vs v12 but +21,979 vs v13. Clean architecture for one-way crashes. |
| r1_v15 | not submitted | — | — | — | v14 + ACO anchor bootstrap (adapts FV from first two-sided market + slow median update, 120-tick window, threshold=12). Intended to protect against IMC shifting FV from 10000. **Synthetic −58,568 vs v14** (fails catastrophically on ACO_CRASH gradient: anchor follows price down, disengages crash_mode, resumes toxic accumulation). Real Day 1 BT: +727 default (small positive on 1k tick window where anchor barely moves), but −6,408 on 4-day 10k aggregate. **Not recommended.** |
| r1_v16 | not submitted | — | — | — | v15 + freeze anchor updates while `prelim_crash` armed (update anchor history and anchor value only when `|cur_mid − anchor| ≤ CRASH_THRESHOLD`). Partially fixes v15: recovers **+22,546 vs v15 on ACO_CRASH** but still **−34,924 vs v14** on that regime. Fundamental issue: gradient crash's initial slow-start ticks don't cross threshold yet, so anchor drifts partially before freeze engages. Total synthetic: 1,741,747 (−37,983 vs v14). |
| r1_v17 | not submitted | — | — | — | v14 + bootstrap-only anchor (one-time snap to first valid two-sided mid, frozen forever). **Beats v14 by +2,748 on 13-regime 25-seed synthetic**. Matches v14 on crash regimes (ACO_CRASH +124, others within noise), wins +1,460/+1,566 on ALT_FV_HIGH/LOW via bootstrap, small baseline cost (~2.9k total) from snap offset. On the largest gap (MID_SHIFT) v16's adaptive helps but v17 still +3,907 vs v14. **New recommendation as EV-optimal default** if FV-shift insurance is desired. |
...
33. **Submission recommendation: r1_v9_defensive is the EV-optimal choice** — v4 (10,624.84) has zero defensive stack and loses ~22k per day vs v9 in a synthetic PERMANENT crash. v9 (10,601.66) keeps the +5 drift startup (full IPR alpha = 7,438) AND adds full ACO defense (circuit breaker + cubic skew + toxic-maker fix = same 3,164 ACO as v10/v11). Break-even vs v4: P(rug) > 23/22,230 = **0.1%**. Any realistic catastrophe probability in a 4-day competition makes v9 strictly better. v10/v11/v12/v14 all cost additional IPR alpha (−146) for no marginal defense benefit on observed data, so they are pareto-dominated by v9 for submission purposes.
34. **All defensive variants v12/v13/v14 produce byte-identical website scores (10,443.78)** — submissions 257139, 258586, 259621 confirm that crash_mode + \|pos\|≥60 state never occurred on Round 1 day-1 real data. The defensive extensions past v9 are purely hypothetical insurance for regimes that haven't materialized. The consistent v10/v11 → v12/v13/v14 drop of −12 is attributable to CRASH_THRESHOLD=15 briefly firing on normal ACO noise (range ±18 on real data), paying a small quote-widening cost without any crash to justify it.
35. **v15 adaptive-anchor architecture fails on gradient crashes** — v15 replaces hardcoded ACO_FV=10000 with a bootstrap-from-first-tick anchor + slow median update (120-tick window, threshold=12). Intent: protect against IMC shifting ACO FV (Superduperbread's concern). **Synthetic 25-seed shows −57,470 ± 2,823 *** on ACO_CRASH regime** (60-tick gradient drop): the adaptive anchor follows price down during the crash, `\|cur_mid − anchor\|` drops below CRASH_THRESHOLD, crash_mode disengages, bot resumes normal trading at crashed prices, accumulates toxic inventory. Small wins on instant crashes (ACO_FLASH +542, PERMANENT +803, CRASH_DEEP +522) because anchor can't adapt in one tick. Total synthetic: −58,568 vs v14. Real Day 1 BT: +727 default (anchor barely moves in 1k ticks). **Lesson:** any adaptive FV mechanism with a lag shorter than the typical crash duration will self-disarm during the crash. For FV-shift insurance without this failure mode, freeze the anchor during crash_mode (update only when NOT in crash_mode).
36. **v16 freeze-during-crash fix is partial, not complete** — v16 adds a preliminary crash check (`|cur_mid − anchor| > CRASH_THRESHOLD`) and skips both anchor history accumulation and anchor update when prelim_crash is True. Synthetic 25-seed: v16 recovers +22,546 vs v15 on ACO_CRASH but still −34,924 vs v14. **Fundamental tension exposed:** at the start of a gradient crash, deviation is below threshold (0-10 ticks), so freeze doesn't engage yet — anchor drifts partially before protection kicks in. Gradient crashes and gradual FV shifts are the same pattern at their initial phase, so no local signal distinguishes them. Total v16 synthetic: 1,741,747 (−37,983 vs v14).
37. **v17 bootstrap-only anchor is the right answer** — instead of adaptive anchor (v15/v16), v17 snaps the anchor once on tick 0 from the first valid two-sided mid, then FREEZES for the entire run. No history tracking, no median update, no drift. **13-regime 25-seed synthetic: v17 beats v14 by +2,748 total**. Matches v14 on all crash regimes (ACO_CRASH +124 essentially identical, PERMANENT/FLASH/DEEP all within noise). Wins +1,460/+1,566 on ALT_FV_HIGH/LOW via bootstrap. Small cost (~2.9k) on baseline regimes from 2-tick snap offset. **v17 architecturally solves the v15/v16 tension**: no ongoing adaptation means no self-disarm on gradients, but the initial bootstrap captures FV-shift insurance.
38. **Surprising finding: v14's hardcoded FV=10000 accidentally works on ALT_FV scenarios** — when market is at 14000, `|cur_mid − ACO_FV=10000| = 4000 > CRASH_THRESHOLD=15` always, so v14's crash_mode is permanently armed, which routes `base_fv = avg_mid` (i.e., tracks the actual market). v14 only loses 1,460 vs v17 on ALT_FV_HIGH because v17 runs in normal mode (tighter edges, full take/clear) while v14 runs in permanent crash mode (wider edges, no take/clear). The crash_mode + anchor-to-avg_mid pattern from v10 is a double-duty insurance: it catches both real crashes AND mislocated hardcoded FVs.
39. **MID_SHIFT (10-tick legitimate shift at tick 5000) is v14's weakest regime (177,388)** — below CRASH_THRESHOLD=15, so crash_mode doesn't engage. v14 persistently misprices by 10 ticks after the shift. v16 adaptive-anchor slowly follows the shift (191,231 = +13,842 vs v14). v17 can't adapt after bootstrap (181,296 = +3,907 vs v14). This is the one regime where adaptive-anchor architecture genuinely helps — but the gradient-crash failure mode makes it net-negative overall.
40. **Final synthetic ranking (13 regimes × 25 seeds, total PnL)**: v17=2,859,598 > v14=2,856,850 > v16=2,834,485. v17 is the EV-optimal choice given it matches v14 on all crash regimes while providing bootstrap FV-shift insurance at ~3k net cost.
...
├── r1_v15.py                            # v14 + adaptive anchor bootstrap. FAILED: synthetic −58,568 vs v14 from anchor-tracks-gradient-crash self-disarm. Not submitted.
├── r1_v16.py                            # v15 + freeze anchor updates during prelim_crash. Partial fix: synthetic +22,546 vs v15 on ACO_CRASH but still −37,983 vs v14 total. Gradient slow-start evades freeze threshold.
├── r1_v17.py                            # v14 + bootstrap-only anchor (snap once at tick 0, freeze forever). 13-regime synthetic: +2,748 vs v14 total. Matches v14 on crash regimes, wins ALT_FV_HIGH/LOW via bootstrap. NEW EV-optimal default.
