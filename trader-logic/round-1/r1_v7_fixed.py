...
"""
r1_v7_fixed — r1_v7 with _wall_mid ask-bug corrected.

Bug in v7/v6b/v6: `min(..., key=lambda p: abs(vol))` picks the SHALLOWEST ask
(smallest |volume|), not the deepest. Nancy's original finds the most-negative
volume — i.e., the largest absolute ask size. This fix restores that.

All other logic is byte-identical to r1_v7 (IPR wall-mid + guardrail, ACO fixed).
"""
