    ...
    def run(self, state):
        ...
        if "TOMATOES" in state.order_depths:
            ...
            if od.buy_orders and od.sell_orders:
                ...
                if len(c)==4: fv=7.39+0.059509*c[0]+0.117116*c[1]+0.243910*c[2]+0.577988*c[3]
                else: fv=mp
                ...
                if self.pb is not None:
                    bc=bb-self.pb
                    if bc>=5: self.sig=-1
                    elif bc<=-5: self.sig=1
                    elif abs(bc)<=1: self.sig*=0.7
                self.pb=bb
                tb=80-pos; ts=80+pos
                mbp=tv-1 if pos>30 else tv; msp=tv+1 if pos<-30 else tv
                ...
                if self.sig>0.5:
                    ...
                    if ts>0: to.append(Order("TOMATOES",max(tv+4,ba-1,bb+1),-ts))
                elif self.sig<-0.5:
                    if ts>0: to.append(Order("TOMATOES",max(tv+1,ba-1),-ts))
                    if tb>0: to.append(Order("TOMATOES",min(tv-4,bb+1,ba-1),tb))
                else:
                    if tb>0: to.append(Order("TOMATOES",min(tv-1,bb+1),tb))
                    if ts>0: to.append(Order("TOMATOES",max(tv+1,ba-1),-ts))
                orders["TOMATOES"]=to
