...
class TomatoesStrategy(Strategy):
    ...
    def act(self, state: TradingState):
        ...
        # Features for Fair Value Prediction
        ...
        # V11 Multi-variate Prediction
        expected_change = (2.6 * obi) - (0.43 * ma_diff) + 2.21
        fv = mid + expected_change
        
        pos = state.position.get(self.symbol, 0)
        to_buy = self.limit - pos
        to_sell = self.limit + pos

        # LAYER 1: Aggressive Liquidity Taking (Directional Edge)
        for price, vol in sells:
            if to_buy > 0 and (fv - price) > (self.half_spread_cost + 1.0):
                qty = min(to_buy, -vol)
                self.buy(price, qty)
                to_buy -= qty
        
        for price, vol in buys:
            if to_sell > 0 and (price - fv) > (self.half_spread_cost + 1.0):
                qty = min(to_sell, vol)
                self.sell(price, qty)
                to_sell -= qty

        # LAYER 2: Passive MM with Inventory Skew
        # We nudge our quotes by 0.1 per unit of inventory to remain 'flat'
        skew = pos * 0.1
        fv_skew = fv - skew

        if to_buy > 0:
            p = min(int(round(fv_skew - 1)), buys[0][0] + 1)
            self.buy(min(p, sells[0][0] - 1), to_buy)

        if to_sell > 0:
            p = max(int(round(fv_skew + 1)), sells[0][0] - 1)
            self.sell(max(p, buys[0][0] + 1), to_sell)
