...
Phase A BT (r4_v4_voucher_obi precursor with OBI_POS_CAP=200):
  1k day 3 probe: $5,586 / 10k default $133,730 / 10k imc $124,284

Phase B sweep refinement (OBI_POS_CAP 200→100):
  1k day 3 probe: $5,246 (-$340 vs cap=200, but +$22 d1, +$120 d2)
  10k 3-day default: $134,200 (+$470)
  10k 3-day imc: $125,763 (+$1,479) ★
  Net across 10k both modes: +$1,949

Other sweeps (all confirmed v5 baseline optimal):
  - HP S17 z-threshold: 2.0 optimal (1.5/1.75/2.25/2.5 all worse)
  - VEV_5200 BS edge: insensitive in [3,7], current 5.0 fine
  - VFE momentum threshold: -3.0 optimal (-2/-4/-5 within noise)
  - OBI threshold: 0.7 optimal (0.5/0.6 too aggressive, 0.8 too tight)
  - OBI_CONFIRM_SCALE: 1/2/3 same outcome (size doesn't bind at high threshold)

STRICTLY PARETO-DOMINATES v3 and v4 on every 10k test window.
1k probe slight regression (-$198 across 3 days) accepted for $1,949 10k gain.
...
OBI_POS_CAP = 100   # v5b: tightened from 200 (sweep: +$1,479 imc / +$470 def / -$340 1k)
...
VFE_MOMO_LOOKBACK = 150     # ticks to look back for velocity
VFE_MOMO_THRESH = -3.0     # mid drop trigger
VFE_MOMO_SIZE = 200        # short size
VFE_MOMO_TP = 9.0          # v5b sweep: was 10, +$508 default / +$469 imc / +$509 1k probe
